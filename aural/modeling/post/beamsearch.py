...
import k2
import torch
from aural.utils.stream import Stream, stack_states, unstack_states
...
class FastBeamSearch:
    ...
    def process(
        ...
        if self.decoding_method == "fast_beam_search_nbest":
            ...
            (next_hyp_list, next_trailing_blank_frames,) = fast_beam_search_nbest_LG(
                model=model,
                encoder_out=encoder_out,
                processed_lens=processed_lens,
                rnnt_decoding_config=rnnt_decoding_config,
            ...
            (next_hyp_list, next_trailing_blank_frames,) = fast_beam_search_one_best(
                model=model,
                encoder_out=encoder_out,
                processed_lens=processed_lens,
                rnnt_decoding_config=rnnt_decoding_config,
            ...
            raise NotImplementedError(f"{self.decoding_method} is not implemented")
...
class GreedySearch:
    ...
    def init_stream(self, stream: Stream):
        ...
        stream.hyp = [self.beam_search_params["blank_id"]] * self.beam_search_params[
            "context_size"
        ]
